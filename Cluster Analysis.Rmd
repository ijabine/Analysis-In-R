...
# The code below is caret way to do scaling
...
```{r}
testdata_new <- cbind(testdata,cut_tree)
testdata_new$cut_tree <- as.factor(testdata_new$cut_tree)
table(testdata_new$cut_tree)
testdata_new %>% ggplot(aes(x = carat,y = price, color = cut_tree)) +
  geom_point()
